...
from app.services.dizimo import criar_transacao_dizimo
from app.services.parcelamento import criar_parcelamento


class TransacaoService:
    def __init__(self, repo: TransacaoRepositoryProtocol | None = None) -> None:
        self._repo: TransacaoRepositoryProtocol = repo if repo is not None else TransacaoRepository()

    def buscar(self, db: Session, transacao_id: int, user_id: int) -> Optional[Transacao]:
        transacao = db.query(Transacao).filter(
            and_(Transacao.id == transacao_id, Transacao.user_id == user_id)
        ).first()
        if transacao:
    ...
    def criar(self, db: Session, transacao: TransacaoCreate, user_id: int) -> Transacao:
        conta = db.query(Conta).filter(
            and_(Conta.id == transacao.conta_id, Conta.user_id == user_id)
        ).first()
        if not conta:
        ...
        if is_parcelado and transacao.total_parcelas and transacao.total_parcelas > 1:
            ...
            return criar_parcelamento(db, transacao, conta, user_id, self._repo)
        ...
        dizimo_criado: Optional[Transacao] = None

        if transacao.tem_dizimo and transacao.tipo == TipoTransacao.ENTRADA:
            self._repo._save(db, db_transacao)  # flush para gerar ID antes de criar o dizimo
            dizimo_criado = criar_transacao_dizimo(db, db_transacao, transacao.percentual_dizimo, user_id)
        else:
            db.add(db_transacao)
        ...
        orcamentos_afetados_simples: set[tuple[int, int, int]] = set()
        if db_transacao.categoria_id and db_transacao.tipo == TipoTransacao.SAIDA:
            orcamentos_afetados_simples.add((db_transacao.categoria_id, db_transacao.data.month, db_transacao.data.year))
        if dizimo_criado and dizimo_criado.categoria_id and dizimo_criado.tipo == TipoTransacao.SAIDA:
            orcamentos_afetados_simples.add((dizimo_criado.categoria_id, dizimo_criado.data.month, dizimo_criado.data.year))
        for categoria_id, mes, ano in orcamentos_afetados_simples:
    ...
    def atualizar(
        ...
        db_transacao = self.buscar(db, transacao_id, user_id)
        if not db_transacao:
            return None
        ...
        dizimo: Optional[Transacao] = None
    ...
    def duplicar(self, db: Session, transacao_id: int, user_id: int) -> Optional[Transacao]:
        original = self.buscar(db, transacao_id, user_id)
        if not original:
            return None
        ...
        if original.parcelado:
            ...
            self._repo._save(db, nova_transacao)
        ...
        return self.criar(db, transacao_copia, user_id)

    def deletar(self, db: Session, transacao_id: int, user_id: int) -> bool:
        db_transacao = self.buscar(db, transacao_id, user_id)
        if not db_transacao:
            return False
        ...
        if db_transacao.tem_dizimo and db_transacao.transacao_dizimo_uuid:
            ...
            if dizimo:
                ...
                if dizimo.categoria_id and dizimo.tipo == TipoTransacao.SAIDA:
                    orcamentos_afetados.add((dizimo.categoria_id, dizimo.data.month, dizimo.data.year))
                self._repo.delete(db, dizimo)

        self._repo.delete(db, db_transacao)
        db.add(conta)
        for meta_id in metas_afetadas:
            recalcular_meta(db, user_id, meta_id)
        for categoria_id, mes, ano in orcamentos_afetados:
